...
public class MarketDataInit {
	...
	public void makeBaseProducts() {
		...
		Product product1 = marketFacade.createProduct(
			user1MarketMember, post1.getModelTypeCode(), post1.getId(), post1.getTitle(), post1.getContent(), 10_000, 10_000
		);

		Product product2 = marketFacade.createProduct(
			user1MarketMember, post2.getModelTypeCode(), post2.getId(), post2.getTitle(), post2.getContent(), 15_000, 15_000
		);

		Product product3 = marketFacade.createProduct(
			user1MarketMember, post3.getModelTypeCode(), post3.getId(), post3.getTitle(), post3.getContent(), 20_000, 20_000
		);

		Product product4 = marketFacade.createProduct(
			user2MarketMember, post4.getModelTypeCode(), post4.getId(), post4.getTitle(), post4.getContent(), 25_000, 25_000
		);

		Product product5 = marketFacade.createProduct(
			user2MarketMember, post5.getModelTypeCode(), post5.getId(), post5.getTitle(), post5.getContent(), 30_000, 30_000
		);

		Product product6 = marketFacade.createProduct(
			user3MarketMember, post6.getModelTypeCode(), post6.getId(), post6.getTitle(), post6.getContent(), 35_000, 35_000
		);
	}
